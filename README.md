  ...
  "cells": [
    ...
    },
    {
      ...
      "outputs": [
        {
          ...
          "text": [
            ...
            "100%|██████████| 19.5M/19.5M [00:00<00:00, 116MB/s] \n"
          ]
        },
        {
          "output_type": "stream",
      ...
      "source": [
        ...
        "# Préfixe à retirer\n",
        "prefix = \"Activité des résidents en 2021 - IRIS - France hors Mayotte_\"\n",
        "\n",
    ...
    },
    {
      ...
      "source": [
        "## **Syntaxe pour supprimer le préfixe des colonnes CSP**"
      ]
    },
    {
      "cell_type": "code",
    ...
    },
    {
      ...
      "source": [
        "# **Isochrone + catégories Socio-Professionelles**"
      ]
    },
    {
      "cell_type": "code",
      ...
      "outputs": [
        {
          ...
          "data": {
            "text/plain": [
              "VBox(children=(HTML(value='<h3>🗺️ Isochrone + Population & CSP estimée</h3>'), FloatText(value=45.75, descript…"
            ],
      ...
      "source": [
        ...
        "\n",
        "# --- Fonction : Population estimée dans l’isochrone ---\n",
        ...
        "                        'Population estimée': int(pop_contrib)\n",
        "                    })\n",
        "                except:\n",
        "                    continue\n",
        "\n",
        ...
        "\n",
        "# --- Fonction : Répartition CSP estimée ---\n",
        ...
        "        'C21_ACT1564_CS2': 'Artisans/Commerçants',\n",
        "        'C21_ACT1564_CS3': 'Cadres',\n",
        "        'C21_ACT1564_CS4': 'Prof. intermédiaires',\n",
        "        'C21_ACT1564_CS5': 'Employés',\n",
        ...
        "\n",
        "# --- Fonction : Génération de la carte + tableaux ---\n",
        ...
        "\n",
        "    # Marqueur central\n",
        "    folium.Marker([lat, lon], icon=folium.Icon(color='red'), popup=\"Point sélectionné\").add_to(fmap)\n",
        "\n",
        "    # Tableaux pour affichage en sortie\n",
        "    df_csp = pd.DataFrame.from_dict(csp_percentages, orient='index', columns=[\"% Actifs\"])\n",
        "    df_csp.index.name = \"Catégorie socio-professionnelle\"\n",
        "    df_csp.reset_index(inplace=True)\n",
        "\n",
        "    df_contrib = pd.DataFrame(communes_contrib)\n",
        "    df_contrib = df_contrib[['Nom IRIS', '% surface', 'Population estimée']]\n",
        "\n",
        ...
        "minutes_widget = widgets.IntSlider(value=10, min=1, max=60, step=1, description='Durée (min):')\n",
        ...
        "        print(f\"📍 Point sélectionné : ({lat}, {lon}) – {minutes} min à pied\\n\")\n",
        "\n",
        "        fmap, df_csp, df_contrib, pop_totale, total_active_population = generate_result_content(lat, lon, minutes, df_popiris)\n",
        "\n",
        "        display(fmap)\n",
        "\n",
        "        display(HTML(f\"<h4>📊 Répartition des CSP (actifs occupés) — Total actifs estimés : {total_active_population:,}</h4>\"))\n",
        "        display(df_csp.style.set_table_attributes('class=\"table\"').format({\"% Actifs\": \"{:.2f}%\"}))\n",
        "\n",
        "# --- Affichage interface ---\n",
        "display(widgets.VBox([\n",
        "    widgets.HTML(\"<h3>🗺️ Isochrone + Population & CSP estimée</h3>\"),\n",
        "    lat_widget,\n",
        "    lon_widget,\n",
        "    minutes_widget,\n",
        "    run_button,\n",
    ...
    },
    {
      ...
      "outputs": [
        {
          ...
          "data": {
            "text/plain": [
              "VBox(children=(HTML(value='<h3>🔍 Comparaison entre IRIS selon une répartition CSP cible</h3>'), HTML(value='<b…"
            ],
            "application/vnd.jupyter.widget-view+json": {
              "version_major": 2,
              "version_minor": 0,
      ...
      "source": [
        "# --- Fonction : Calcul des % CSP dans chaque IRIS (limité au Rhône) ---\n",
        ...
        "\n",
        "    # Filtrage : ne garder que les IRIS du département du Rhône (code INSEE 69)\n",
        ...
        "\n",
        "# --- Fonction : Trouver les IRIS les plus proches de la répartition cible ---\n",
        ...
        "\n",
        "# --- Interface widgets (sans durée) ---\n",
        "csp_widgets = {\n",
        "    'Cadres': widgets.IntSlider(value=20, min=0, max=100, step=1, description='Cadres'),\n",
        "    'Employés': widgets.IntSlider(value=20, min=0, max=100, step=1, description='Employés'),\n",
        "    'Ouvriers': widgets.IntSlider(value=20, min=0, max=100, step=1, description='Ouvriers'),\n",
        "    'Prof. intermédiaires': widgets.IntSlider(value=20, min=0, max=100, step=1, description='Intermédiaires'),\n",
        "    'Artisans/Commerçants': widgets.IntSlider(value=10, min=0, max=100, step=1, description='Artisans'),\n",
        ...
        "        if total_pct == 0:\n",
        "            print(\"⚠️ La somme des pourcentages CSP ne peut pas être 0.\")\n",
        "            return\n",
        "\n",
        "        # Normalisation\n",
        "        csp_target = {k: round((v / total_pct) * 100, 2) for k, v in csp_target.items()}\n",
        "        print(\"🎯 Répartition CSP cible (normalisée à 100%) :\", csp_target)\n",
        "\n",
        "        # Calcul des répartitions CSP par IRIS (limité au Rhône)\n",
        ...
        "\n",
        "        display(HTML(\"<h4>📍 IRIS avec la répartition CSP la plus proche (Rhône uniquement) :</h4>\"))\n",
        "        for i, row in closest.iterrows():\n",
        "            print(f\"- Score : {round(row['score'], 2)} – Actifs : {int(row['total_actifs'])}\")\n",
        ...
        "# --- Affichage Interface ---\n",
        "display(widgets.VBox([\n",
        "    widgets.HTML(\"<h3>🔍 Comparaison entre IRIS selon une répartition CSP cible</h3>\"),\n",
        "    widgets.HTML(\"<b>Définissez votre répartition cible :</b>\"),\n",
        "    *csp_widgets.values(),\n",
        "    run_button,\n",
        "    output\n",
        "]))\n",
    ...
    },
    {
      ...
      "source": [
        ...
        "        closest = find_closest_iris_to_csp_target(csp_target, df_csp, csp_labels, top_n=5)\n",
        "\n",
        "        display(HTML(\"<h4>📊 Tableau des 5 IRIS les plus proches :</h4>\"))\n",
        "\n",
        "        # 🔵 Affichage du tableau avec nom_iris ajouté\n",
      ...
      "outputs": [
        {
          ...
          "data": {
            "text/plain": [
              ...
              "0  0000   Saint-Marcel-d'Urf�     42255        42   \n",
              "1  0000  Saint-Pierre-de-B�uf     42272        42   \n",
              "\n",
              "                                geo_point_ nom_dept               nom_com  \\\n",
              "0  (2:45.87073884180357,3.879745794829265)    LOIRE   Saint-Marcel-d'Urf�   \n",
              "1   (2:45.37716722880891,4.74461200054728)    LOIRE  Saint-Pierre-de-B�uf   \n",
              "\n",
    ...
    },
    {
      ...
      "outputs": [
        {
          ...
          "data": {
            "text/plain": [
              "VBox(children=(HTML(value='<h3>🔍 Trouver les IRIS idéaux selon votre profil CSP</h3>'), SelectMultiple(descrip…"
            ],
            "application/vnd.jupyter.widget-view+json": {
              "version_major": 2,
              "version_minor": 0,
      ...
      "source": [
        ...
        ")\n",
        "duration_selector = widgets.IntSlider(value=10, min=1, max=30, step=1, description='Durée (min)')\n",
        "\n",
        "csp_widgets = {\n",
        "    'Cadres': widgets.IntSlider(value=20, min=0, max=100, step=1, description='Cadres'),\n",
        "    'Employés': widgets.IntSlider(value=20, min=0, max=100, step=1, description='Employés'),\n",
        ...
        "}\n",
        "\n",
        "run_button = widgets.Button(description=\"Trouver les IRIS idéaux\", button_style='success')\n",
        "output = widgets.Output()\n",
        "\n",
        "# --- Callback principal ---\n",
        "def on_run_clicked(b):\n",
        ...
        "        if not communes:\n",
        "            print(\"❌ Veuillez sélectionner au moins une commune.\")\n",
        "            return\n",
        "\n",
        ...
        "        csp_target = {k: round((v / total_pct) * 100, 2) for k, v in csp_target.items()}\n",
        "\n",
        "        print(f\"🎯 Répartition cible : {csp_target}\")\n",
        "        print(f\"📍 Communes sélectionnées : {', '.join(communes)} — Durée max : {duree_min} min (~{max_distance} m)\")\n",
        "\n",
        "        df_zone = df_popiris[df_popiris['nom_com'].isin(communes)].copy() # Corrected column name\n",
        "        if df_zone.empty:\n",
        "            print(\"❌ Aucune IRIS trouvée dans les communes sélectionnées.\")\n",
        "            return\n",
        "\n",
        "        df_csp, csp_labels = compute_iris_csp_distribution(df_zone)\n",
        "\n",
        "        # Distance à partir du centre moyen de la sélection\n",
        ...
        "            print(\"❌ Aucune IRIS accessible dans la durée de marche indiquée.\")\n",
        "            return\n",
        "\n",
        "        closest = find_closest_iris_to_csp_target(csp_target, df_filtered, csp_labels).head(5)\n",
        "\n",
        "        display(HTML(\"<h4>📊 Top 5 IRIS dans les communes sélectionnées :</h4>\"))\n",
        ...
        "\n",
        "        best = closest.iloc[0]\n",
        "        print(f\"✅ Meilleure correspondance : {best['nom_iris']} à {best['nom_com']} (score : {round(best['score'],2)}, distance : {int(best['distance'])} m)\") # Corrected column name\n",
        "\n",
        ...
        "update_zone_options()\n",
        "display(widgets.VBox([\n",
        "    widgets.HTML(\"<h3>🔍 Trouver les IRIS idéaux selon votre profil CSP</h3>\"),\n",
        "    commune_selector,\n",
        "    duration_selector,\n",
        "    widgets.HTML(\"<b>Définissez votre répartition cible :</b>\"),\n",
        "    *csp_widgets.values(),\n",
    ...
    },
    {
      ...
      "outputs": [
        {
          ...
          "name": "stdout",
          "text": [
            "📍 Analyse autour de : (45.75, 4.85)\n",
            "🕒 Durée sélectionnée : 6 min à pied\n",
            "🎯 Cible CSP normalisée : {'C21_ACT1564_CS1': 5.0, 'C21_ACT1564_CS2': 10.0, 'C21_ACT1564_CS3': 15.0, 'C21_ACT1564_CS4': 20.0, 'C21_ACT1564_CS5': 30.0, 'C21_ACT1564_CS6': 20.0}\n",
            "\n"
          ]
        },
        {
      ...
      "source": [
        ...
        "    else:\n",
        "        return pd.DataFrame()  # Aucun IRIS trouvé\n",
        "\n",
        "# --- Fonction : Génération de la carte + résultats ---\n",
        ...
        "\n",
        "# --- Widgets pour la répartition cible ---\n",
        "csp_sliders = {\n",
        ...
        "    'C21_ACT1564_CS4': widgets.FloatSlider(min=0, max=100, value=20, step=1, description=\"Intermédiaires\"),\n",
        "    'C21_ACT1564_CS5': widgets.FloatSlider(min=0, max=100, value=30, step=1, description=\"Employés\"),\n",
        "    'C21_ACT1564_CS6': widgets.FloatSlider(min=0, max=100, value=20, step=1, description=\"Ouvriers\")\n",
        "}\n",
        "\n",
        "# --- Widgets pour coord et durée ---\n",
        "lat_widget = widgets.FloatText(value=45.75, description='Latitude :')\n",
        "lon_widget = widgets.FloatText(value=4.85, description='Longitude :')\n",
        "minutes_widget = widgets.IntSlider(value=10, min=1, max=60, description=\"Durée (min)\")\n",
        "\n",
        ...
        "        if total == 0:\n",
        "            print(\"⚠️ La somme des pourcentages doit être > 0\")\n",
        "            return\n",
        "        cible_csp = {k: (v / total) * 100 for k, v in cible_csp.items()}\n",
        "\n",
        "        print(f\"📍 Analyse autour de : ({lat}, {lon})\")\n",
        "        print(f\"🕒 Durée sélectionnée : {minutes} min à pied\")\n",
        "        print(f\"🎯 Cible CSP normalisée : {cible_csp}\\n\")\n",
        "\n",
        "        map_out, df_scores = generate_result_content(lat, lon, minutes, df_popiris, cible_csp)\n",
        "        display(map_out)\n",
        "\n",
        "        if df_scores.empty:\n",
        "            display(HTML(\"<b style='color:red;'>❌ Aucun IRIS trouvé dans la zone d'isochrone.</b>\"))\n",
        "            return\n",
        "\n",
        "        display(HTML(\"<h4>🔎 IRIS les plus proches du profil cible :</h4>\"))\n",
        "        display(df_scores.head(10).style.set_table_attributes('class=\"table table-striped\"').format({'Score': '{:.2f}'}))\n",
        "\n",
        "# --- Affichage interface ---\n",
        "run_button.on_click(on_button_click)\n",
        "\n",
        "display(widgets.HTML(\"<h3>📍 Saisissez les coordonnées du point à analyser</h3>\"))\n",
        "display(widgets.HBox([lat_widget, lon_widget]))\n",
        "display(minutes_widget)\n",
        "display(widgets.HTML(\"<h4>🎯 Définir une répartition cible CSP (%)</h4>\"))\n",
    ...
    },
    {
      ...
      "source": [
        ...
        "    return response.json()\n",
        "\n",
        "# --- Fonction : Carte et IRIS intersectés ---\n",
        ...
        "\n",
        "# --- Chargement des données IRIS avec CSP ---\n",
        "df_popiris = gpd.read_file(\"iris_population.geojson\")\n",
        "\n",
        "# --- Widgets coordonnées et durée ---\n",
        "lat_widget = widgets.FloatText(value=45.4397, description='Latitude :')  # Saint-Étienne\n",
        ...
        "            display(HTML(\"<b style='color:red;'>❌ Aucun IRIS trouvé dans la zone d'isochrone.</b>\"))\n",
        "        else:\n",
        "            display(HTML(f\"<h4>✅ {len(df_intersected)} IRIS intersectés :</h4>\"))\n",
        "\n",
        "            # Colonnes CSP à afficher\n",
        "            csp_columns = [\n",
        "                'C21_ACT1564_CS1',  # Agriculteurs\n",
        "                'C21_ACT1564_CS2',  # Artisans, commerçants\n",
        "                'C21_ACT1564_CS3',  # Cadres\n",
        ...
        },
        {
          ...
          "data": {
            ...
            "text/html": [
              ...
              "        \n",
              "            \n",
              "                var html_401dddf50d7a01355257db77c1997a85 = $(`&lt;div id=&quot;html_401dddf50d7a01355257db77c1997a85&quot; style=&quot;width: 100.0%; height: 100.0%;&quot;&gt;Point sélectionné&lt;/div&gt;`)[0];\n",
    ...
    },
    {
      ...
      "source": [
        ...
        "            display(HTML(f\"<h4>✅ {len(df_intersected)} IRIS intersectés :</h4>\"))\n",
        "\n",
        "            # Colonnes CSP à afficher (à adapter si noms différents)\n",
        "            csp_columns = [\n",
        "                'C21_ACT1564_CS1',  # Agriculteurs\n",
        "                'C21_ACT1564_CS2',  # Artisans\n",
        "                'C21_ACT1564_CS3',  # Cadres\n",
        "                'C21_ACT1564_CS4',  # Intermédiaires\n",
        "                'C21_ACT1564_CS5',  # Employés\n",
        "                'C21_ACT1564_CS6',  # Ouvriers\n",
        "            ]\n",
        "\n",
    ...
    },
    {
      ...
      "source": [
        ...
        "\n",
        "ORS_API_KEY = '5b3ce3597851110001cf6248ab2580d70bc148e6b2da078f1f784bf1'  # Ta clé ORS\n",
        "\n",
        "def get_isochrone_from_coords(lat, lon, minutes, api_key=ORS_API_KEY):\n",
        "    url = 'https://api.openrouteservice.org/v2/isochrones/foot-walking'\n",
        "    headers = {\n",
        ...
        "                csp_values = {}\n",
        "                try:\n",
        "                    # Calcul des valeurs pondérées par la surface intersectée\n",
        "                    for col_code, csp_name in csp_cols.items():\n",
        ...
        "        lon = lon_widget.value\n",
        "        minutes = minutes_widget.value\n",
        "        print(f\"📍 Point : ({lat}, {lon}) – {minutes} min à pied\\n\")\n",
        "\n",
        "\n",
        ...
        "            return\n",
        "\n",
        "        display(HTML(f\"<h4>📋 CSP des IRIS intersectant l'isochrone (en %)</h4>\"))\n",
        ...
        "\n",
        "display(widgets.VBox([\n",
        "    widgets.HTML(\"<h3>🗺️ Isochrone et CSP par IRIS</h3>\"),\n",
        "    lat_widget,\n",
        "    lon_widget,\n",
        "    minutes_widget,\n",
        "    run_button,\n",
    ...
    },
    {
      ...
      "source": [
        ...
        "            return\n",
        "\n",
        "        # Création et affichage de la carte Folium avec l'isochrone\n",
        "        fmap = folium.Map(location=[lat, lon], zoom_start=14)\n",
        "        folium.Marker(location=[lat, lon], tooltip=\"Point de départ\").add_to(fmap)\n",
    ...
    },
    {
      ...
      "source": [
        ...
        "\n",
        "                rows.append({\n",
        "                    'Nom IRIS': row.get('nom_iris', 'Inconnu'),  # <-- Modifié ici\n",
        "                    '% Surface dans isochrone': round(ratio * 100, 2),\n",
        "                    **csp_values\n",
        "                })\n",
        "\n",
    ...
    },
    {
      ...
      "source": [
        ...
        "        fmap = folium.Map(location=[lat, lon], zoom_start=14)\n",
        "        # Point de départ\n",
        "        folium.Marker([lat, lon], tooltip=\"Point de départ\").add_to(fmap)\n",
        "        # Polygone isochrone\n",
      ...
      "application/vnd.jupyter.widget-state+json": {
        ...
        "7d0de0e535d24d60aecb3a8474ccbb5f": {
          ...
          "state": {
            ...
            "value": "<h3>🗺️ Isochrone + Population & CSP estimée</h3>"
          }
        },
        "783cefef97eb4b42bc0b557cc6b92d6c": {
          "model_module": "@jupyter-widgets/controls",
            ...
            "outputs": [
              {
                ...
                "name": "stdout",
                "text": [
                  "🎯 Répartition CSP cible (normalisée à 100%) : {'Cadres': 20.0, 'Employés': 20.0, 'Ouvriers': 20.0, 'Prof. intermédiaires': 20.0, 'Artisans/Commerçants': 10.0, 'Agriculteurs': 10.0}\n"
                ]
              },
              {
                "output_type": "display_data",
                "data": {
                  ...
                  "text/html": "<h4>📍 IRIS avec la répartition CSP la plus proche (Rhône uniquement) :</h4>"
                },
                "metadata": {}
              },
              {
        ...
        "1bee9d65db264865b7ca28fcab894825": {
          ...
          "state": {
            ...
            "value": "<h3>🔍 Comparaison entre IRIS selon une répartition CSP cible</h3>"
          }
        },
        "ffaa2ca05a1a49449fb730efe7c5c1fa": {
          "model_module": "@jupyter-widgets/controls",
          ...
          "state": {
            ...
            "value": "<b>Définissez votre répartition cible :</b>"
          }
        },
        "1b7a45008abc40618fd5b97a670711f7": {
          "model_module": "@jupyter-widgets/controls",
            ...
            "outputs": [
              {
                ...
                "name": "stdout",
                "text": [
                  "🎯 Répartition cible : {'Cadres': 34.43, 'Employés': 16.39, 'Ouvriers': 16.39, 'Prof. intermédiaires': 16.39, 'Artisans/Commerçants': 8.2, 'Agriculteurs': 8.2}\n",
                  "📍 Communes sélectionnées : Aigueperse — Durée max : 5 min (~400 m)\n"
                ]
              },
              {
                "output_type": "stream",
              ...
              },
              {
                ...
                "data": {
                  "text/plain": "<IPython.core.display.HTML object>",
                  "text/html": "<h4>📊 Top 5 IRIS dans les communes sélectionnées :</h4>"
                },
                "metadata": {}
              },
              {
              ...
              },
              {
                ...
                "name": "stdout",
                "text": [
                  "✅ Meilleure correspondance : Aigueperse à Aigueperse (score : 84.83, distance : 0 m)\n"
                ]
              },
              {
                "output_type": "display_data",
        ...
        "fbb0fa09b2324bd9b47438a13065044f": {
          ...
          "state": {
            ...
            "value": "<h3>🔍 Trouver les IRIS idéaux selon votre profil CSP</h3>"
          }
        },
        "270fc3b4a3fe46fe84f927fd51da176a": {
          "model_module": "@jupyter-widgets/controls",
          ...
          "state": {
            ...
            "_options_labels": [
              ...
              "Drac�",
              "Duerne",
              "D�bats-Rivi�re-d'Orpra",
              "D�cines-Charpieu",
              "Essertines-en-Ch�telneuf",
              "Essertines-en-Donzy",
              "Estivareilles",
              "Farnay",
              "Feurs",
              ...
              "Saint-Andr�-la-C�te",
              "Saint-Andr�-le-Puy",
              "Saint-And�ol-le-Ch�teau",
              "Saint-Appolinaire",
              "Saint-Appolinard",
              "Saint-Barth�lemy-Lestra",
              "Saint-Bonnet-de-Mure",
              "Saint-Bonnet-des-Bruy�res",
              "Saint-Bonnet-des-Quarts",
              "Saint-Bonnet-le-Ch�teau",
              ...
              "Saint-Christophe",
              "Saint-Cl�ment-de-Vers",
              "Saint-Cl�ment-les-Places",
              "Saint-Cl�ment-sur-Valsonne",
              "Saint-Cyprien",
              "Saint-Cyr-au-Mont-d'Or",
              "Saint-Cyr-de-Favi�res",
              "Saint-Cyr-de-Valorges",
              "Saint-Cyr-le-Chatoux",
              "Saint-Cyr-les-Vignes",
              "Saint-Cyr-sur-le-Rh�ne",
              ...
              "Saint-Loup",
              "Saint-L�ger-sur-Roanne",
              "Saint-Mamert",
              "Saint-Marcel-d'Urf�",
              "Saint-Marcel-de-F�lines",
              "Saint-Marcel-l'�clair�",
              "Saint-Marcellin-en-Forez",
              "Saint-Martin-Lestra",
              "Saint-Martin-d'Estr�aux",
              "Saint-Martin-en-Haut",
              "Saint-Martin-la-Plaine",
              "Saint-Martin-la-Sauvet�",
              "Saint-Maurice-en-Gourgois",
              "Saint-Maurice-sur-Dargoire",
              "Saint-Michel-sur-Rh�ne",
              "Saint-M�dard-en-Forez",
              "Saint-Nizier-d'Azergues",
              "Saint-Nizier-de-Fornas",
              "Saint-Nizier-sous-Charlieu",
              "Saint-Paul-d'Uzore",
              "Saint-Paul-de-V�zelin",
              ...
              "Saint-V�rand",
              "Saint-�tienne",
              "Saint-�tienne-des-Oulli�res",
              "Saint-�tienne-la-Varenne",
              "Saint-�tienne-le-Molard",
        ...
        "34d1a19a82ff4d62b2f60c9695ef1b2c": {
          ...
          "state": {
            ...
            "_view_name": "ButtonView",
            "button_style": "success",
            "description": "Trouver les IRIS idéaux",
            "disabled": false,
            "icon": "",
            ...
            "outputs": [
              {
                ...
                "name": "stdout",
                "text": [
                  "🎯 Répartition cible : {'Cadres': 20.0, 'Employés': 20.0, 'Ouvriers': 20.0, 'Prof. intermédiaires': 20.0, 'Artisans/Commerçants': 10.0, 'Agriculteurs': 10.0}\n",
                  "📍 Communes sélectionnées : Affoux, Aigueperse, Alix, Ambierle — Durée max : 10 min (~800 m)\n"
                ]
              },
              {
                "output_type": "stream",
              ...
              },
              {
                ...
                "name": "stdout",
                "text": [
                  "✅ Meilleure correspondance : Alix à Alix (score : 34.92, distance : 0 m)\n"
                ]
              },
              {
                "output_type": "display_data",
        ...
        "f204ea8621aa487b930d7aa96e87fea0": {
          ...
          "state": {
            ...
            "value": "<h3>📍 Saisissez les coordonnées du point à analyser</h3>"
          }
        },
        "2c40ddd19d6a4f36b62c10c683144ce6": {
          "model_module": "@jupyter-widgets/base",
        ...
        "7e6d2c959de046bda306eb289b3c1c25": {
          ...
          "state": {
            ...
            "value": "<h4>🎯 Définir une répartition cible CSP (%)</h4>"
          }
        },
        "2d21274638f54fc7a430ff16f6984c56": {
          "model_module": "@jupyter-widgets/base",
            ...
            "outputs": [
              {
                ...
                "name": "stdout",
                "text": [
                  "📍 Point sélectionné : (45.4397, 4.3872) – 10 min à pied\n",
                  "\n"
                ]
              },
              {
